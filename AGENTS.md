...
- Frontend: `frontend/` (Vue 3 + Vite + TS). Verify with `npm run lint`, `npm run test:unit -- --run`, `npm run build`.
- Backend: `backend/` (.NET 10, controllers). Verify with `dotnet test` and `dotnet format --verify-no-changes`.
